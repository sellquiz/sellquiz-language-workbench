        ...
        for (let i = 0; i < input.length; i++) {
            ...
            if (x.startsWith('%')) {
                ...
                for (const item of unorderedItems)
                    content +=
                        '<li>' + this.compile_paragraph(item, co) + '</li>';
                content += '</ul>';
                unorderedItems = [];
            } else if (
            ...
            // centered equation
                ...
                let name = '',
                    label = '';
                [name, label] = this.extract_name_and_label(
                    x.substring(1).trim(),
                );
                if (label.length > 0) {
                ...
                if (parsingBox == false) {
                    if (boxType === 'sell') {
                        ...
                        p.type =
                            boxType === 'plot2d'
                                ? plot.PlotType.Plot2d
                                : plot.PlotType.PlotTikz;
                        p.id = co.plots.length;
                        p.title = '';
                        p.src = '';
                        const lines = box.split('\n');
                        for (let i = 0; i < lines.length; i++) {
                            if (i == 0) p.title = lines[i];
                            else p.src += lines[i] + '\n';
                        }
                        co.plots.push(p);
                        ...
                        if (parts['error'].length > 0) {
                            ...
                            p.type =
                                boxType === 'javablock'
                                    ? prog.ProgrammingQuizType.JavaBlock
                                    : prog.ProgrammingQuizType.Python;
                            p.id = co.programmingQuizzes.length;
                            p.title = parts['@title'];
                            p.text = this.compile(parts['@text'], false).html;
                            p.given = this.removeEmptyLines(parts['@given']);
                            p.asserts = this.removeEmptyLines(
                                parts['@asserts'],
                            ).split('\n');
                            p.forbiddenKeywords = this.removeEmptyLines(
                                parts['@forbidden-keywords'],
                            ).split('\n');
                            p.requiredKeywords = this.removeEmptyLines(
                                parts['@required-keywords'],
                            ).split('\n');
                            p.solution = parts['@solution'];
                            co.programmingQuizzes.push(p);
                            content +=
                                '<div id="programming-' + p.id + '"></div>\n';
                        }
