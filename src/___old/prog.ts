...
export class ProgrammingQuiz {
    ...
    evaluate() {
        ...
        const feedback_htmlElement = document.getElementById(
            'programming-feedback-' + this.id,
        );
        const wait_text = lang.text('please_wait');
        feedback_htmlElement.innerHTML =
            '<span class="text-danger">' + wait_text + '</span>';
        axios
            .post(
                service_url,
                new URLSearchParams({
            ...
            .then(function (response) {
                ...
                feedback_htmlElement.innerHTML =
                    (status === 'ok' ? lang.checkmark : lang.crossmark) + ' ';
                feedback_htmlElement.innerHTML +=
                    ' &nbsp; <code>' +
                    message
                        .replaceAll('\n', '<br/>')
    ...
    updateHTML() {
        ...
        html +=
            '<span class="h2 py-1 my-1">' +
            '<i class="fas fa-keyboard"></i> ' +
            this.title +
            '</span><br/>\n';
        html += this.text;
        html +=
            '<div class="border p-0 m-0"><textarea class="form-control p-0" style="min-width: 100%;" id="programming-editor-' +
            this.id +
            '" + rows="5"></textarea></div>';
        html += '<p id="programming-feedback-' + this.id + '"></p>';
        const evalStr = lang.text('evaluate');
        html +=
            '<button type="button" class="btn btn-primary" onclick="slw.eval_prog(\'' +
            this.id +
            '\')">' +
            evalStr +
            '</button>' +
