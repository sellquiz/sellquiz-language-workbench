...
var Question = class {
  ...
  compileQuestion(part, numberOfInstances) {
    ...
    if ("options" in part.labeledText) {
      this.isPseudoQuestion = part.labeledText["options"].includes("hide-button");
    }
...
var Document = class {
  ...
  compile(input2, oldOutput2) {
    ...
    for (let lineIdx = 0; lineIdx < n; lineIdx++) {
      ...
      if (line.trim().length == 0) {
        ...
        else if (line.startsWith("!lang=EN"))
          this.language = "EN";
      } else if (line.startsWith("# ")) {
        const part = new Part(lineIdx + 1);
        this.parts.push(part);
        ...
        if (rest.startsWith("*"))
          part.text = rest.substring(1);
        else
          part.text = this.section + ". " + rest;
        this.section++;
        this.subsection = 1;
        this.susubsection = 1;
      } else if (line.startsWith("## ")) {
  ...
  compileTheorem(part) {
    part.type = "theorem" /* theorem */;
    part.text = this.compileParagraph(part.labeledText[""]);
  }
  compileProof(part) {
    part.type = "proof" /* proof */;
    part.text = this.compileParagraph(part.labeledText[""]);
  }
  compileExample(part) {
  ...
  compileQuestion(part) {
    ...
    part.question.compileQuestion(part, 10);
